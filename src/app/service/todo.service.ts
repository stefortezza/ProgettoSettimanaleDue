...
export class TodoService {
  ...
  getTodosByUserId(userId: number): Observable<Todo[]> {
    return this.getTodos().pipe(
      map(todo => todo.filter(todo => todo.userId === userId))
    );
  }
  ...
  updateTodoLocal(todo: Todo): Observable<Todo> {
    const index = this.todo.findIndex(t => t.id === todo.id);
    if (index !== -1) {
      this.todo[index] = { ...this.todo[index], ...todo };
      return new Observable(observer => {
        observer.next(this.todo[index]);
        observer.complete();
      });
    }
